    ...
    def handle(self, *args, **options):
        ...
        while processed < options["batch_size"]:
            with transaction.atomic():
                job = PerformanceRecalculation.objects.select_for_update(skip_locked=True).filter(status__in=(PerformanceRecalculation.Status.PENDING, PerformanceRecalculation.Status.FAILED)).order_by("created_at").first()
                if not job: break
